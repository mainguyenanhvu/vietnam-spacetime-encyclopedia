...
| 10 | Sa đồ chiến dịch | 🔄 **UI + trình dựng xong, dữ liệu 36/189.** `battle.ts` 631 → 1012 dòng, trình dựng SVG TỔNG QUÁT chạy bằng dữ liệu — soạn sa đồ mới chỉ còn là viết JSON. Bản vẽ tay Bạch Đằng giữ nguyên. `battles/_index.json` sinh tự động + cổng `validate_sado_index.mjs`. **+35 sa đồ trải năm 40 → 1979.** Vá 38 chỗ nhãn đè (phép đo cũ mù với nhãn mũi tên). **Còn lại**: 153/189 trận chưa có; danh sách ứng viên đợt 3 đã chốt với người soạn. |
