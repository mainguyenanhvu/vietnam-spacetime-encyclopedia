...
**Đợt 2026-08-27 — mở rộng 16 agent song song, hai ca.** Lớp phủ **2.903 → 3.511 (+608)**, chạm **32/35 lớp**; sa đồ **240 → 267, phủ kín 267/267**; trích văn tịch **167 → 242 đoạn / 90 → 153 trận**, tất cả khớp byte. Ba lớp không chạm: `ban-do-co` (cần ảnh quét), `unesco` (đã đủ), `khoa-bang-danh-nhan` (167 mục, đã dày). Chi tiết ở `RELEASE.md`.

🟢 **Ca chiều đảo được kết luận của ca sáng về hạng mục rủi ro.** Ca sáng tự soát → xếp danh hiệu nhà nước là rủi ro số 1. Ca chiều soát chéo bằng bậc 3 → **danh hiệu là hạng mục ĐÚNG nhất** (một khối đúng 17/17; hai khối khác 1/56 và 1/21 sai), còn lỗi dồn sang địa chỉ cấp thôn, số đo, tuổi. Lý do: danh hiệu có **văn bản gốc kiểm được** (Quyết định Chủ tịch nước / Bộ VHTTDL kèm danh sách tên), còn «thôn Dê Dàng» hay «86 tuổi» thì không có văn bản nào để đối chiếu. **Hạng mục nguy hiểm không phải hạng mục quan trọng nhất, mà là hạng mục KHÔNG CÓ NGUỒN ĐÓNG để đối chiếu.** Xếp lại thứ tự soát cho đợt sau theo nguyên tắc đó, đừng chép lại bảng của ca sáng.

⚠️ **grep `entries-index.json` KHÔNG đủ khi nhiều agent chạy song song** — index là ảnh chụp cũ, không thấy file vừa bị sửa sống trên đĩa. `validate_overlays.mjs` quét file sống mới là cổng thật. Ba file bị hai agent cùng ghi trong đợt này; các agent tự phát hiện và tự rút.

🔴 **Bài học lớn nhất của đợt, đọc trước khi mở đợt nạp dữ liệu tiếp theo.** Mọi agent tự soát lại việc mình VỪA BÁO LÀ XANH CẢ 4 CỔNG đều tìm ra lỗi thật: **22% · 23% · 31% · 31% · 38% · 67%**. Cơ chế KHÔNG phải công cụ bịa mà là **mô hình tự điền từ trí nhớ tham số rồi tưởng đã đọc thấy trong nguồn** — nguyên văn lời một agent: *«tê giác một sừng tuyệt chủng 2010 — tôi tự thêm chi tiết đó từ kiến thức nền, không có trong bất kỳ trang nào tôi mở.»*
Hệ quả bắt buộc: **cấp ngân sách cho một lượt tự soát ngay trong brief**, coi là một pha của công việc chứ không phải việc thêm; và **soát chéo bởi agent khác mạnh hơn tự soát** (nó bắt được lỗi hệ thống «điền năm hiện tại cho mọi sự kiện không rõ năm» mà agent viết không tự thấy).
Hạng mục rủi ro xếp theo thứ tự để định trọng tâm soát: (1) **danh hiệu/giải thưởng nhà nước** — ba ca bịa độc lập trong một đêm, và đây là loại sai về danh dự người thật · (2) năm · (3) số đo/thành tích · (4) địa chỉ cấp xã · (5) **liên hệ nhân quả giữa hai dữ kiện đều đúng** — loại gần như không ai bắt được.
⚠️ **Nhiều nguồn trùng khớp KHÔNG phải bằng chứng độc lập**: ba bản tóm tắt WebSearch khớp nhau chia chung một nguồn sai là mô hình. Chỉ tính là đối chiếu khi mỗi nguồn là một TRANG ĐÃ MỞ.
⚠️ **«Trang trống» / «bị chặn» là triệu chứng của CÔNG CỤ, không phải bằng chứng về NỘI DUNG.** Đã có hai ca: `mod.gov.vn` đặt cookie rồi `window.location.reload()`, và `qdnd.vn` trả 302 vòng chờ cookie — cả hai làm curl thấy trống trong khi trang đầy đủ. **Chỉ xoá mục khi** tên miền không phân giải được (`nslookup` VÀ `curl` cùng xác nhận) **hoặc** trang mở được nhưng không nói về đối tượng. Timeout/SSL hết hạn/404/anti-bot → giữ mục, gắn `ghi_chu_bien_tap`.
...
- [~] **(cũ) 129 khẳng định — mô tả gốc:** Đã đếm bằng `git diff` trên dòng thêm: anh-hung-can-hien-dai 56 · danh-nhan-van-hoa 29 · tri-thuc-khoa-hoc 21 · dtts 10 · danh-y 7 · nghe-nhan 5 · bao-tang 1. Agent nhận việc chạm giới hạn phiên trước khi trả kết quả. Riêng `anh-hung-can-hien-dai` có trục thứ hai: **trường `loai` tự nó là khẳng định danh hiệu** (`anh-hung-llvt` tuyên bố đã được phong) — mục nào nguồn không xác nhận PHONG phải chuyển `bieu-tuong-khang-chien`, theo mẫu `lam-son-nao`. **M**
- [x] ✅ **Lượt tự soát — XONG HẾT 2026-08-27 chiều.** `quansu-vuachua` 43 mục (46,5% sai, giữ đủ 4 mục Đội Hoàng Sa sau khi cắt chi tiết bịa ở cả 4) · `tinnguong-tonghe` 53/53 (17%) · `khoabang-suthan` +15 sứ thần +9 huyền sử · `lehoi-truyenthuyet` 4 file, +59 mục · `anhhung-llvt` vẫn chưa báo. Chi tiết ở `RELEASE.md`.
- [~] **(cũ) mô tả gốc:** `quansu-vuachua` (43 mục, chưa soát — trọng tâm 4 mục Đội Hoàng Sa Lý Sơn vì là bề mặt pháp lý) · `tinnguong-tonghe` (36/53 mục chưa soát) · `khoabang-suthan` (chưa báo cáo lần nào) · `lehoi-truyenthuyet` (chưa báo cáo) · `anhhung-llvt` đợt 2 (chưa báo cáo).
- [x] ✅ **3 mục `chisi-canvuong` — CỨU LẠI ĐƯỢC CẢ BA, 2026-08-27.** `dao-nguyen-pho` (SSL hết hạn + nginx trả 404 giả cho non-browser; domain sống ở 27.71.228.229 — thay nguồn baophapluat.vn, và phát hiện nguồn nhà nước tự vênh năm mất 1907/1908, đã nêu cả hai theo bất biến #4) · `le-khiet` (404 một URL, domain sống — thay nguồn trường THPT Chuyên Lê Khiết, sửa «Nhâm Ngọ» → «Canh Ngọ 1882») · `nguyen-van-giap` (NXDOMAIN thật nhưng tìm được nguồn thay của GS Đinh Xuân Lâm — sửa luôn chi tiết nơi mất vốn đến từ WebSearch). Cả ba viết lại dày hơn bản cũ.
- [~] **(cũ) mô tả gốc:**: `dao-nguyen-pho` (anti-bot chặn), `le-khiet` (404 một URL), `nguyen-van-giap` (DNS fail chưa xác nhận `nslookup`). Ca `doi-van` thì **giữ nguyên quyết định xoá** — archives.org.vn crawl được, trang chỉ nhắc tên một lần thoáng qua, toàn bộ câu chuyện chi tiết là dựng lên.
...
- [ ] 🔴 **Đếm lại 2026-08-28: KHÔNG phải ~10 mục — là 94, và tất cả nằm trong MỘT chỗ.** `anh-hung-can-hien-dai.json` chứa **94 mục `loai: "me-vnah"`, id đều bắt đầu `me-vnah-*`**, trong khi lớp `me-vnah.json` riêng chỉ có **65**. Không trùng một người nào (0 trùng id, 0 trùng tên) — tức đây là **đặt nhầm lớp**, không phải nhân bản. Hệ quả người dùng thấy: bật lớp «Mẹ VNAH» ra **65/159** Mẹ, 94 Mẹ còn lại chỉ hiện khi bật lớp anh hùng.
  Chính `id` và `loai` của 94 mục đã tự khai đích đến, nên bằng chứng đủ mạnh; nhưng chuyển 94 mục giữa hai lớp là **đổi thứ người dùng thấy dưới lớp nào**, nên chờ chủ dự án chốt. Việc thi hành thì cơ học: cắt-dán giữa hai file, id vốn đã duy nhất nên không va, rồi sinh lại chỉ mục theo đúng thứ tự. **M**
...
- 🔴 **`baohoabinh.com.vn` ĐÃ ĐỔI CHỦ.** Mọi URL — kể cả trang chủ — trả **HTTP 200 với đúng 44.810 byte** của **«HỘI NHÀ BÁO TỈNH BẮC NINH»**, một ấn phẩm khác hẳn. Kho bài Báo Hoà Bình không còn ở đó. Đây tệ hơn 404: mã HTTP xanh nên mọi phép kiểm URL đều lọt. Đừng cử agent vào lại.
- 🔴 **Nhánh chính `qdnd.vn` / `www.qdnd.vn` chặn từ môi trường này** — 302 lặp vô hạn, cả curl lẫn WebFetch, cả `http` lẫn `https`, cả bản có và không có `www`. **`sknc.qdnd.vn` (Sự kiện & Nhân chứng) thì mở được** — dùng nhánh đó.
- **Anh hùng người dân tộc thiểu số qua công cụ tìm kiếm: đã chạm đáy** sau 5 vòng. Các con số tổng xác nhận CÓ người (163 anh hùng DTTS toàn quốc · 393 cá nhân Quân khu 4 · 9 cá nhân Gia Lai), nhưng danh sách TÊN kèm dân tộc chưa được số hoá công khai, trừ hai trang hiếm (Sở VHTTDL Điện Biên, Công an Quảng Ngãi) đã khai thác hết. Chăm và Ê Đê: hai vòng tìm theo họ đặc trưng, 0 kết quả. Muốn thêm phải có bộ sách *Anh hùng người dân tộc thiểu số* (NXB Chính trị quốc gia) hoặc kho lưu trữ ngoài Google-index.
