...
- [ ] 2.122 di tích lịch sử văn hoá — **CHIẾN DỊCH MỞ 2026-08-11.** Sàng trùng-tên-chính-xác toàn DB: còn **2.114**. Phân bố dày nhất: Hà Nội 597 · Huế 138 · Hải Dương cũ 118 · TP.HCM 104 · Thái Bình cũ 91 · Nghệ An 91. **Wave 1 = 3 lô × 45** (Hà Nội, Huế, Hải Dương cũ) đang chạy — quy trình: agent tra nguồn (dsvh > cổng tỉnh > báo Đảng, cấm bịa số QĐ) + trường `cap` phân luồng: `quoc-gia` → nạp `di-tich-quoc-gia.json`, `cap-tinh`/`chua-ro` → kho `docs/backlog/wave1-cap-tinh-vetted.json` **chờ chủ dự án quyết có mở lớp «di tích cấp tỉnh» không** (câu hỏi mới cho §11). Bẫy đã dặn agent: hạng mục thuộc cụm Cố đô Huế / Côn Sơn–Kiếp Bạc không tách bản ghi. **L**
