...
| 6 | Mốc dòng thời gian: gắn vị trí, hiện note khi kéo; đủ 4000 năm | 🔄 **UI XONG, nội dung mới phủ 6/13 đoạn.** `src/moc-lich-su.ts` + `.css` mới: 62 vạch đính theo năm, note nhỏ bung khi kéo, ◂ ▸ đi hết mốc trong kỳ, bấm vạch nhảy đúng thời kỳ. Đo trên Chrome thật: vạch năm 602 rơi 491,1 px / lý thuyết 491,1 px — **lệch 0**. Màu vạch 8,11–12,12:1 ở cả hai chế độ. **Còn lại**: xem khoảng trống đo được ngay dưới. |
| 7 | Ngục trung nhật ký xếp theo thứ tự bài | ✅ Xong |
| 8 | Thư viện: chế độ đọc kiểu Kindle, chế độ lật, mở rộng cửa sổ, tuỳ chỉnh, bỏ biểu tượng thừa, sắp theo chủ đề + thời gian/tác giả, link trang tỉnh, bổ sung tác phẩm | 🔄 **Dữ liệu nền đã xong, UI chưa dựng.** 560/560 mục cũ có `chu_de`; `_chu-de.json` 12 chủ đề; +44 tác phẩm; 2 file mới `su-ky-dia-chi.json`, `van-xuoi-hien-thuc-vung-mien.json`. Nay đã CÓ trục để sắp theo chủ đề — trước không có. **Còn lại**: toàn bộ UI đọc (chế độ lật, tuỳ chỉnh, bỏ icon thừa) theo `dacta_thu_vien.md`; sắp theo tác giả/thời gian; `jumpToTinh(slug)`. |
...
| 10 | Sa đồ chiến dịch | 🔄 **UI xong, trình dựng xong, dữ liệu 16/189.** `battle.ts` 631 → 1012 dòng: thêm trình dựng SVG TỔNG QUÁT chạy bằng dữ liệu — soạn sa đồ mới nay chỉ còn là viết JSON, không đụng TypeScript. Bản vẽ tay của Bạch Đằng giữ nguyên (`sa_do_kieu` vắng → nhánh cũ). Bỏ hằng `SA_DO_SAN_CO` chép cứng, thay bằng `battles/_index.json` sinh tự động + cổng `validate_sado_index.mjs` chống mốc. +14 sa đồ nội dung trải năm 40 → 1975. Nghiệm thu Chrome thật: 15/15 đủ bước, console sạch. **Còn lại**: 173/189 trận chưa có sa đồ — đợt 2 (20 trận) đã giao. |
| 11 | Lớp phủ chia theo lĩnh vực/chủ đề/giai đoạn, **không chia theo giới tính** | 🔄 đang phân loại 38 mục của `nu-danh-nhan-lich-su`. Đo được: `danh-nhan-quan-su-co-trung-dai.json` **vốn đã có 8 mục `loai: "nu-tuong"`** → hai lớp trùng trục sẵn, rủi ro trùng người là thật chứ không phải giả định. Nhóm `nu-dan-toc` trong `overlays-config.ts` sẽ giải thể sau khi phân bổ xong. |
