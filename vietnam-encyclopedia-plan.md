...
- ✅ **R8** (commit 81e7a69): `src/battle.ts` sa đồ SVG tương tác trận Bạch Đằng 938 — bước qua 5 giai đoạn, toggle nhóm phần tử + chỉ báo thuỷ triều; `bach-dang-938.json` theo ĐVSKTT+SGK, disclaimer «không theo tỉ lệ», draft. `validate_battles.mjs` (CI).
- ✅ **R6** (81e7a69): `src/journey.ts` hành trình hoá thân 6 chặng ngôi-hai (dựng nước→Bạch Đằng→dời đô→chống Nguyên Mông→Lam Sơn→Đống Đa), mỗi chặng mount nhân vật 3D + link sa đồ trận; `hanh-trinh.json` nguồn chính sử. `validate_journey.mjs` (CI) ép liên kết chéo figure_id/battle_id.
- Verified: `tsc && vite build` xanh; **7 validator CI** (chủ quyền/tỉnh/văn học/ảnh/nhân vật/trận/hành trình) pass. **Chưa soi mắt render** (automation browser bị bóp rAF) — cần Iron Man eyeball.
- 🎉 **TOÀN BỘ 10 HẠNG MỤC XONG** trong 1 phiên (R2–R9 ship; R1 quy trình; R10 ongoing). Tất cả nội dung sử = draft chờ NGƯỜI KIỂM SỬ (cổng §9). Việc cần người: điền youtube_id 32 bài hát; sinh ảnh AI cho mục thiếu (môi trường không có tool); duyệt draft nhân vật/trận/hành trình; eyeball render 3D.
